...
public class TideDebugLog
{
	 ...
	 * @since jEdit 2.6pre5
	 */
	public static final int MAXLINES = 5000;
	...
	public static void init(boolean stdio, int level)
	{
		...
		TideDebugLog.level = level;

	} //}}}

	//{{{ setTideDebugLogWriter() method
	/**
	 ...
	 * @since jEdit 3.2pre4
	 */
	public static void setTideDebugLogWriter(Writer stream)
	{
		if(TideDebugLog.stream == null && stream != null)
		{
			try
			{
				if(wrap)
	...
	//{{{ getTideDebugLogListModel() method
	/**
	 * Returns the list model for viewing the log contents.
	 * @since jEdit 4.2pre1
	 */
	public static ListModel getTideDebugLogListModel()
	{
		return listModel;
	} //}}}
	...
	//{{{ log() method
	/**
	 * TideDebugLogs an exception with a message.
	 *
	...
	//{{{ log() method
	/**
	 * TideDebugLogs a message. This method is thread-safe.<p>
	 *
	 * The following code sends a typical debugging message to the activity
	 * log:
	 * <pre>TideDebugLog.log(TideDebugLog.DEBUG,this,"counter = " + counter);</pre>
	 ...
	 * <code>TideDebugLog.DEBUG</code>, <code>TideDebugLog.MESSAGE</code>,
	 * <code>TideDebugLog.NOTICE</code>, <code>TideDebugLog.WARNING</code>, or
	 * <code>TideDebugLog.ERROR</code>.
	...
	private static final TideDebugLogListModel listModel;
	...
	static
	{
		...
		listModel = new TideDebugLogListModel();
	...
	private static PrintStream createPrintStream(final int urgency,
		final Object source)
	{
		return new TideDebugLogPrintStream(urgency, source);
	} //}}}
	...
	//{{{ TideDebugLogListModel class
	static class TideDebugLogListModel implements ListModel
	{
	...
	//{{{ TideDebugLogPrintStream class
	/**
	 * A print stream that uses the "TideDebugLog" class to output the messages,
	 ...
	 */
	private static class TideDebugLogPrintStream extends PrintStream {

		private final ByteArrayOutputStream buffer;
		private final OutputStream orig;

		//{{{ TideDebugLogPrintStream constructor
		TideDebugLogPrintStream(int urgency, Object source)
		{
			super(new TideDebugLogOutputStream(urgency, source));
	...
	//{{{ TideDebugLogOutputStream class
	private static class TideDebugLogOutputStream extends OutputStream
	{
		private final int 	urgency;
		private final Object 	source;

		//{{{ TideDebugLogOutputStream constructor
		TideDebugLogOutputStream(int urgency, Object source)
		{
