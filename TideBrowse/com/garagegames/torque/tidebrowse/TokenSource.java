/*
 * TokenSource.java - Token producer for a given TideBrowseParser.LineSource
 *
...
// that are made available by a given TideBrowseParser.LineSource object.
public class TokenSource {
    private TideBrowseParser.LineSource ls;
     ...
     */
    TokenSource(TideBrowseParser.LineSource ls) {
     ...
     */
    TokenSource(TideBrowseParser.LineSource ls, int debugLevel) {
...
//%                         }
                            ...
                            if (tokenizerOffset >= lineString.length()) {
                                ...
                                // linesource is already exhausted
                                if (ls.isExhausted()) {
                                    // The file ends with a multi-line comment
                                    return null;
                                }

                                lineString = getNextLine();
                                if (lineString == null) {
                                    // not very nice, this may happen if we end
                                    // with a comment.
